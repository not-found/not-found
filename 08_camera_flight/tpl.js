var app = (function() {
    ...
    var illumination = {
        ambientLight : [ .5, .5, .5 ],
        light : [
            {isOn : true, position : [ 3., 1., 3. ], color : [ 1., 1., 1. ], zAngle: 0.0},
            {isOn : true, position : [ -3.,1.,-3. ], color : [ 1., 1., 1. ], zAngle: 0.0}
        ]
    };
    ...
    function initEventHandler() {
        ...
        window.onkeydown = function(evt) {
            ...
            // Light move and orbit.
            switch (c) {
            case ('I'):
                // Orbit camera.
                illumination.light[0].zAngle += sign * deltaRotate;
                break;
            case ('L'):
                // Move camera up and down.
                illumination.light[1].zAngle += sign * deltaRotate;
                break;
            }
    ...
    function calculateLightOrbit()
    {
        var x = 0, z = 2;
        for(var i = 0; i < illumination.light.length; i++)
        {
            illumination.light[i].position[x] = camera.center[x];
            illumination.light[i].position[z] = camera.center[z];
            illumination.light[i].position[x] += camera.distance * Math.sin(illumination.light[i].zAngle);
            illumination.light[i].position[z] += camera.distance * Math.cos(illumination.light[i].zAngle);
        }
    }
